...
// Ingest tests task
gulp.task('test-ingest', function (done) {
  process.env.NODE_ENV = 'test';
  var testSuites = ['scripts/ingest/ingest-death-records.test.js'];
  var error;
...
gulp.task('ingest', function (cb) {
  var error;
  var ingestFile = arg.file || arg.f;
  var timeout = arg.timeout || arg.t;
  var command = ['node', './scripts/ingest/ingest-death-records.js', ingestFile, timeout];
  // using spawn so that it dumps out the console live.
  var node_spawn = spawn(command[0], command.splice(1));
  node_spawn.stdout.on('data', function (data) {
    console.log(data.toString());
  });

  node_spawn.stderr.on('data', function (data) {
    console.log(data.toString());
  });

  node_spawn.on('exit', function (code) {
...
// task for exporting mongo records from the db in a CSV format
gulp.task('export_to_csv', function (cb) {
  var error;
  var outFile = arg.file || arg.f || `./death-records-export-${moment().format('YYYY_MM_DD-hh_mm_ss')}.csv`;
  console.log(`attempting to export death-records to ${outFile}`);
  var command = ['node', './scripts/ingest/export-death-records.js', outFile];
  var node_spawn = spawn(command[0], command.splice(1));
  node_spawn.stdout.on('data', function (data) {
    console.log(data.toString());
  });
...
gulp.task('test', function (done) {
  runSequence('env:test', 'test:server', 'test-ingest', 'karma', 'nodemon', 'protractor', done);
});
